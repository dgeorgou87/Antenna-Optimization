function [GlobalParams,GlobalMin] = COA(ProblemFunction, DisplayFlag, ProbFlag, RandSeed)
%% ------------------------------------------------------------------------
...
%% Main loop
year=0;
while nfeval<OPTIONS.MaxFEs % Stopping criteria
    ...
    for p=1:n_packs
        ...
        for c=1:n_coy_aux
            ...
            Population(c).chrom=new_coyotes(c,:);
            
            temp1=OPTIONS.popsize;
            OPTIONS.popsize=1;
            % Population = FeasibleFunction(OPTIONS, Population);
            Population(c) = CostFunction(OPTIONS, Population(c));

            new_cost=Population(c).cost;
        ...
        temp1=OPTIONS.popsize;
        OPTIONS.popsize=1;
        PopulationPup.chrom=pup;
        % Population = FeasibleFunction(OPTIONS, Population);
        PopulationPup = CostFunction(OPTIONS, PopulationPup);
        pup_cost=PopulationPup.cost;
        OPTIONS.popsize=temp1;
