function Make_Patch_2_best

fC = [868e6 1800e6 2100e6];
freqlist=fC;
freqlist=freqlist/1e9; %covert to GHz
freqname=['Patch_2_SSA_',int2str(freqlist(1)*1000),'MHz_',int2str(freqlist(2)*1000),'MHz_',int2str(freqlist(3)*1000),'MHz_'];

% Add paths to the required m-files.
hfssSetExePath;
tmpPrjFile = [pwd, '\Patch_2_SSA.aedt'];
tmpScriptFile = [pwd, '\Patch_2_SSA.vbs'];
...
%SSA x=[35.565775, 52.539472, 0.197045, 0.068670, 0.262419, 0.393504, 0.462396, 0.134368, 0.241316, 0.076997, 0.643543, 0.663924, 0.143841, 0.370259];
x=[36.340000, 47.950000, 0.160000, 0.050000, 0.110000, 0.160000, 0.150000, 0.380000, 0.260000, 0.260000, 0.310000, 0.640000, 0.120000, 0.380000];
W=x(1);
L=x(2);
